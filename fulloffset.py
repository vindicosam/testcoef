...
class LidarCameraVisualizer:
    def __init__(self):
        ...
        # Initialize with 0 offset for all segments and ring types
        for segment in range(1, 21):
            self.segment_radial_offsets[segment] = {
                'doubles': 0.0,  # mm, positive = away from bull, negative = toward bull
                'trebles': 0.0,
                'small': 0.0,    # inner single area
                'large': 0.0     # outer single area
            }
    ...
    def apply_segment_coefficients(self, x, y):
        ...
        if dist <= self.radii["outer_bull"]:
            ring_type = "small"  # Use small for bull area
        elif dist < self.radii["inner_treble"]:
            ring_type = "small"  # Inner single area
        elif dist < self.radii["outer_treble"]:
            ring_type = "trebles"  # Triple ring area
        elif dist < self.radii["inner_double"]:
            ring_type = "large"  # Outer single area
        elif dist < self.radii["outer_double"]:
            ring_type = "doubles"  # Double ring area
        else:
        ...
        # Save segment as string for coefficient lookup
        seg_str = str(segment)
    ...
    # NEW: Updated method to set segment radial offset with ring type option
    def set_segment_radial_offset(self, segment, offset_mm, ring_type='all'):
        """
        Set a radial offset for a specific segment and ring type.
        
        Args:
            segment (int): Segment number (1-20)
            offset_mm (float): Offset in mm (positive = away from bull, negative = toward bull)
            ring_type (str): 'doubles', 'trebles', 'small', 'large', or 'all'
        """
        if 1 <= segment <= 20:
            if ring_type.lower() == 'all':
                # Apply to all ring types
                for rt in ['doubles', 'trebles', 'small', 'large']:
                    self.segment_radial_offsets[segment][rt] = offset_mm
                print(f"Set segment {segment} radial offset to {offset_mm} mm for all ring types")
            elif ring_type.lower() in ['doubles', 'trebles', 'small', 'large']:
                self.segment_radial_offsets[segment][ring_type.lower()] = offset_mm
                print(f"Set segment {segment} radial offset to {offset_mm} mm for {ring_type}")
            else:
                print(f"Invalid ring type: {ring_type}. Must be 'doubles', 'trebles', 'small', 'large', or 'all'")
        else:
            print(f"Invalid segment number: {segment}. Must be between 1-20.")
            
    # NEW: Updated method to get segment radial offset with ring type option
    def get_segment_radial_offset(self, segment, ring_type=None):
        """
        Get the radial offset for a specific segment.
        
        Args:
            segment (int): Segment number (1-20)
            ring_type (str): 'doubles', 'trebles', 'small', 'large', or None for all
                
        Returns:
            float or dict: Offset in mm for specific ring_type or dict of all ring types
        """
        if 1 <= segment <= 20:
            if ring_type is None:
                return self.segment_radial_offsets[segment]
            elif ring_type.lower() in ['doubles', 'trebles', 'small', 'large']:
                return self.segment_radial_offsets[segment][ring_type.lower()]
            else:
                print(f"Invalid ring type: {ring_type}. Must be 'doubles', 'trebles', 'small', 'large', or None")
                return 0.0
        else:
            print(f"Invalid segment number: {segment}. Must be between 1-20.")
            return 0.0
    ...
    def load_segment_radial_offsets(self, filename="segment_offsets.json"):
        ...
        try:
            if os.path.exists(filename):
                ...
                # Convert string keys back to integers for segments
                # But preserve string keys for ring types
                self.segment_radial_offsets = {
                    int(segment): {
                        ring_type: offset for ring_type, offset in ring_data.items()
                    } for segment, ring_data in loaded_offsets.items()
                }
                
                # Ensure all segments and ring types exist
                for segment in range(1, 21):
                    if segment not in self.segment_radial_offsets:
                        self.segment_radial_offsets[segment] = {}
                    
                    for ring_type in ['doubles', 'trebles', 'small', 'large']:
                        if ring_type not in self.segment_radial_offsets[segment]:
                            self.segment_radial_offsets[segment][ring_type] = 0.0
    ...
    def _calibrate_segment_radial_offsets(self):
        ...
        for segment in range(1, 21):
            ...
            for ring_type in ['doubles', 'trebles', 'small', 'large']:
                offset = self.segment_radial_offsets[segment][ring_type]
                direction = "toward bull" if offset < 0 else "away from bull"
                if offset == 0:
                    direction = "no offset"
                print(f"  {ring_type.capitalize()}: {abs(offset):.1f} mm {direction}")
        
        print("\nEnter commands in format: [segment]:[ring_type]:[offset] ")
        ...
        print("Example: 3:trebles:11 - Moves segment 3 trebles 11mm away from bull")
        print("Example: 6:doubles:-5 - Moves segment 6 doubles 5mm toward bull")
        print("Example: all:all:0 - Resets all segments and ring types to no offset")
        ...
        while True:
            ...
            try:
                parts = cmd.split(':')
                if len(parts) != 3:
                    print("Invalid format. Use segment:ring_type:offset")
                    continue
                    
                segment_str, ring_type_str, offset_str = parts
                ...
                if ring_type_str.lower() == 'all':
                    ring_types = ['doubles', 'trebles', 'small', 'large']
                elif ring_type_str.lower() in ['doubles', 'trebles', 'small', 'large']:
                    ring_types = [ring_type_str.lower()]
                else:
                ...
                for segment in segments:
                    for ring_type in ring_types:
                        self.segment_radial_offsets[segment][ring_type] = offset
                
                direction = "toward bull" if offset < 0 else "away from bull"
                if offset == 0:
                    direction = "no offset"
                    
                print(f"Updated offset for {len(segments)} segment(s) and {len(ring_types)} ring type(s) to {abs(offset):.1f} mm {direction}")
