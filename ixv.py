...
class LidarCameraVisualizer:
    def __init__(self):
        ...
        self.camera_vector_length = 800      # INCREASED: Vector length in mm to 800mm
    ...
    def signal_handler(self, signum, frame):
        """Handle CTRL+C to gracefully shut down the program."""
        self.running = False
        print("\nShutting down...")
        plt.close("all")
        sys.exit(0)
    ...
    def measure_tip_angle(self, mask, tip_point):
        """
        Measure the angle of the dart tip using the enhanced approach.
        Uses RANSAC fitting for more robust angle estimation with improved noise handling.
    ...
    def camera_detection(self):
        """
        Detect dart tip using the camera (left position) with improved settings.
        The camera detects the dart tip’s angle and board Y-position.
        """
        ...
        while self.running:
            ...
            if contours:
                ...
                if tip_contour is not None and tip_point is not None:
                    ...
                    cv2.drawContours(roi_vis, [tip_contour], 0, (0, 255, 0), 2)
                    cv2.circle(roi_vis, tip_point, 5, (0, 0, 255), -1)
                    if dart_angle is not None:
                        angle_rad = math.radians(90 - dart_angle)
                        ...
                        if dart_angle < 85:
                            ...
                            lean = "RIGHT"
                        cv2.putText(roi_vis, f"Angle: {dart_angle:.1f}° ({lean})", 
                    ...
                    if dart_mm_y is not None:
                        cv2.putText(roi_vis, f"Board Y: {dart_mm_y:.1f}mm", 
                                    (10, roi_vis.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
            ...
            frame_vis[self.camera_roi_top:self.camera_roi_bottom, self.camera_roi_left:self.camera_roi_right] = roi_vis
            cv2.rectangle(frame_vis, (self.camera_roi_left, self.camera_roi_top), (self.camera_roi_right, self.camera_roi_bottom), (0, 255, 255), 1)
            # Uncomment these for debugging:
            # cv2.imshow("Camera Feed", frame_vis)
    ...
    def find_camera_board_intersection(self, camera_y):
        ...
        Since the camera is on the left, we assume the board surface is at x=0.
    ...
    def calculate_final_tip_position(self, camera_point, lidar1_point, lidar2_point):
        """
        Calculate the final tip position using all available data.
        If only one LIDAR is available, use that LIDAR point with camera Y adjustment.
        ...
        if len(valid_lidar_points) == 1:
            if camera_point is not None and self.camera_data["dart_angle"] is not None:
                ...
                if side_lean_angle < 85:
                    lean_factor = (85 - side_lean_angle) / 85.0
                    y += -lean_factor * self.side_lean_max_adjustment
                elif side_lean_angle > 95:
                    lean_factor = (side_lean_angle - 95) / 85.0
                    y += lean_factor * self.side_lean_max_adjustment
        ...
        if camera_point is not None:
            ...
            final_tip_position = valid_lidar_points[0]
